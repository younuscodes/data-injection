...
var RdsPostgresql rdsPostgres

func (r rdsPostgres) Insert() {
	log.Println("AWS postgres Insert triggered....")
	// init a Mysql connection
	//dsn := "postgres://younus:Nafeezyou9@localhost:5432/crud"
	dsn := "postgres://" + os.Getenv("AWS_POSTGRES_USER") + ":" + os.Getenv("AWS_POSTGRES_PASSWORD") + "@" + os.Getenv("AWS_POSTGRESL_HOST") + ":" + os.Getenv("AWS_POSTGRES_PORT") + "/" + os.Getenv("AWS_POSTGRES_DB")
	...
	table = os.Getenv("AWS_POSTGRES_TABLE")
	...
	if isArray {
		...
		log.Println("Inserting ducuments to aws postgres............")
...
func Worker(id string, lines chan map[string]interface{}, wg *sync.WaitGroup, db *gorm.DB) {
	...
	for data := range lines {
		...
		if i%BatchSize == 0 {
			//fmt.Println("my I value eache batch", i)
			...
			/*type kaman_data struct{}
			_ = db.Model(&kaman_data{}).Create(
				msg,
			)*/
			//fmt.Printf("%s inserts: %d documents", id, i)
			//fmt.Println()
			if i%50000 == 0 {
